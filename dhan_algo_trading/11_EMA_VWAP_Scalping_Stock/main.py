#!/usr/bin/env python3
"""Dhan EMA + VWAP Scalping Algo for NSE equity.
...
The strategy is alignment-based, not a one-bar crossover event:

    LONG  = fast EMA > slow EMA  AND  price > session VWAP
            + optional bullish candle  + optional volume confirmation
    SHORT = fast EMA < slow EMA  AND  price < session VWAP
            + optional bearish candle  + optional volume confirmation

TP and SL are armed from the actual fill price (percentage or points).
A confirmed TP or SL exit stops the bot for that run.
"""
...
WHY_NO_TRADE_LABELS = {
    ...
    "INDICATORS_INVALID": "Indicator data is invalid — no trade",
    "EMA_NOT_ALIGNED": "Fast and slow EMA are not aligned for an entry",
    "PRICE_BELOW_VWAP": "Price is below session VWAP",
    "PRICE_ABOVE_VWAP": "Price is above session VWAP",
    "CANDLE_NOT_BULLISH": "Last completed candle is not bullish",
    "CANDLE_NOT_BEARISH": "Last completed candle is not bearish",
    ...
    "SCANNING": "Flat — scanning for EMA + VWAP alignment",
...
def parse_cli_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="EMA + VWAP Scalping Algo for Dhan")
    parser.add_argument(
        "--config",
...
def validate_config(config: dict) -> None:
    ...
    session = config.get("trading_session")
    if not isinstance(session, dict):
        session = config.get("trading_hours")
        if not isinstance(session, dict):
            raise ConfigError(
                "CONFIGURATION ERROR\ntrading_session section is required"
            )
        config["trading_session"] = session
    config["trading_hours"] = session
    ...
    polling = _as_positive_number(bot.get("polling_seconds", 5), "bot.polling_seconds")
    if polling < 1:
        raise ConfigError("CONFIGURATION ERROR\npolling interval must be >= 1")
    bot["polling_seconds"] = polling
    bot["candle_lookback"] = int(
        _as_positive_number(bot.get("candle_lookback", 80), "bot.candle_lookback")
    )
    bot["order_wait_seconds"] = _as_positive_number(
        bot.get("order_wait_seconds", execution.get("order_timeout_seconds", 15)),
        "bot.order_wait_seconds",
    )
    ...
    if not isinstance(ema, dict):
        ema = {}
        strategy["ema"] = ema
    fast_period = int(_as_positive_number(ema.get("fast_period", 9), "strategy.ema.fast_period"))
    slow_period = int(_as_positive_number(ema.get("slow_period", 21), "strategy.ema.slow_period"))
    if fast_period >= slow_period:
        raise ConfigError(
            "CONFIGURATION ERROR\nfast_period must be less than slow_period"
        )
    ema["fast_period"] = fast_period
    ema["slow_period"] = slow_period
    ...
    entry["require_ema_alignment"] = _as_bool(entry.get("require_ema_alignment"), True)
    entry["require_vwap_confirmation"] = _as_bool(
        entry.get("require_vwap_confirmation"), bool(vwap["enabled"])
    )
    entry["candle_confirmation"] = _as_bool(entry.get("candle_confirmation"), True)
    entry["minimum_volume_multiplier"] = _as_non_negative_number(
        entry.get("minimum_volume_multiplier", 1.0),
        "strategy.entry.minimum_volume_multiplier",
    )
    entry["volume_lookback"] = int(
        _as_positive_number(entry.get("volume_lookback", 20), "strategy.entry.volume_lookback")
    )

    exit_cfg = strategy.get("exit")
    if not isinstance(exit_cfg, dict):
        exit_cfg = {}
        strategy["exit"] = exit_cfg
    exit_cfg["opposite_signal"] = _as_bool(exit_cfg.get("opposite_signal"), True)

    needed = max(slow_period + 2, entry["volume_lookback"] + 1, 10)
    if bot["candle_lookback"] < needed:
        raise ConfigError(
            f"CONFIGURATION ERROR\nbot.candle_lookback ({bot['candle_lookback']}) is too small "
            f"for the configured EMA / volume windows (need at least {needed})"
        )
    ...
    max_trades = int(
        _as_positive_number(
            risk.get("max_trades_per_day", risk.get("max_daily_trades", 10)),
            "risk.max_trades_per_day",
        )
    )
    if max_trades < 1:
        raise ConfigError("CONFIGURATION ERROR\nmax trades must be >= 1")
    risk["max_trades_per_day"] = max_trades
    risk["max_daily_trades"] = max_trades
    ...
    risk["stop_bot_after_tp_sl"] = _as_bool(
        risk.get("stop_bot_after_tp_sl", risk.get("stop_bot_after_exit", True)),
        True,
    )
    risk["stop_bot_after_exit"] = risk["stop_bot_after_tp_sl"]
    ...
    entry_side = str(execution.get("entry_side") or "").strip().upper()
    if not entry_side:
        enable_long = _as_bool(execution.get("enable_long", True), True)
        enable_short = _as_bool(execution.get("enable_short", False), False)
    ...
    if entry_side not in VALID_ENTRY_SIDES:
        raise ConfigError("CONFIGURATION ERROR\nentry_side must be BOTH, LONG, or SHORT")
    if entry_side in {"BUY"}:
        entry_side = "LONG"
    if entry_side in {"SELL"}:
        entry_side = "SHORT"
    execution["entry_side"] = entry_side
    execution["enable_long"] = entry_side in {"BOTH", "LONG"}
    execution["enable_short"] = entry_side in {"BOTH", "SHORT"}
    ...
    if not execution["enable_long"] and not execution["enable_short"]:
        raise ConfigError("CONFIGURATION ERROR\nat least one entry side must be enabled")
    ...
    execution["order_timeout_seconds"] = _as_positive_number(
        execution.get("order_timeout_seconds", bot["order_wait_seconds"]),
        "execution.order_timeout_seconds",
    )
    bot["order_wait_seconds"] = execution["order_timeout_seconds"]
    execution["prevent_duplicate_orders"] = _as_bool(
        execution.get(
            "prevent_duplicate_orders",
            safety.get("prevent_duplicate_orders", True),
        ),
        True,
    )
    ...
    if not session.get("stop_entry_time"):
        session["stop_entry_time"] = session.get("stop_new_entries_time", "15:00")
    if not session.get("close_positions_time"):
        session["close_positions_time"] = session.get(
            "close_all_positions_time", "15:15"
        )
    session["stop_new_entries_time"] = session["stop_entry_time"]
    session["close_all_positions_time"] = session["close_positions_time"]
    start_h, start_m = _parse_hhmm(session.get("start_time", "09:20"), "trading_session.start_time")
    entry_h, entry_m = _parse_hhmm(
        session.get("stop_entry_time", "15:00"), "trading_session.stop_new_entries_time"
    )
    close_h, close_m = _parse_hhmm(
        session.get("close_positions_time", "15:15"),
        "trading_session.close_all_positions_time",
    )
    if session["enabled"]:
        if (start_h, start_m) >= (entry_h, entry_m):
            raise ConfigError(
                "CONFIGURATION ERROR\nstart_time must be before stop_new_entries_time"
            )
        if (entry_h, entry_m) > (close_h, close_m):
            raise ConfigError(
                "CONFIGURATION ERROR\nstop_new_entries_time must be at or before close_all_positions_time"
            )
    ...
    position_management["exit_on_opposite_signal"] = _as_bool(
        position_management.get(
            "exit_on_opposite_signal",
            strategy["exit"].get("opposite_signal", True),
        ),
        True,
    )
    strategy["exit"]["opposite_signal"] = position_management["exit_on_opposite_signal"]
    ...
    shutdown["close_positions_on_manual_stop"] = _as_bool(
        shutdown.get(
            "close_positions_on_manual_stop",
    ...
    shutdown["close_positions_on_session_end"] = _as_bool(
        shutdown.get(
            "close_positions_on_session_end",
    ...
    shutdown["close_positions_on_tp_sl"] = _as_bool(
        shutdown.get(
            "close_positions_on_tp_sl",
            safety.get("close_positions_on_tp_sl", True),
        ),
        True,
    )
    shutdown["close_positions_on_error"] = _as_bool(
        shutdown.get(
            "close_positions_on_error",
            safety.get("close_positions_on_error", False),
        ),
        False,
    )
    shutdown["cancel_pending_orders"] = _as_bool(
        shutdown.get(
            ...
            safety.get("cancel_pending_orders_on_shutdown", True),
        ),
        True,
    )
    safety["close_positions_on_shutdown"] = shutdown["close_positions_on_manual_stop"]
    safety["cancel_pending_orders_on_shutdown"] = shutdown["cancel_pending_orders"]
    safety["max_api_retries"] = int(
        _as_positive_number(safety.get("max_api_retries", 3), "safety.max_api_retries")
    )
    safety["retry_delay_seconds"] = _as_positive_number(
        safety.get("retry_delay_seconds", 2), "safety.retry_delay_seconds"
    )
...
def calculate_indicators(candles: list[Candle], config: dict) -> dict[str, Any]:
    """
    Compute fast EMA, slow EMA, session VWAP, and volume ratio.

    Parameters:
        candles: completed bars, oldest first.
        config: validated YAML.

    Returns:
        Dict with ema_fast, ema_slow, vwap, volume, average_volume,
        volume_ratio, close, candle, or empty values when data is insufficient.

    Why this function exists:
        Indicator math is separated from signal rules so the CLI can show
        the numbers even when no trade is allowed. We never trade on
        invalid indicators.
    """
    empty = {
        "ema_fast": None,
    ...
    if not candles:
        return empty
    strategy = config["strategy"]
    fast_period = int(strategy["ema"]["fast_period"])
    slow_period = int(strategy["ema"]["slow_period"])
    ...
    current_volume, average_volume, volume_ratio = calculate_volume_ratio(
        volumes, int(strategy["entry"]["volume_lookback"])
    )
    ...
    valid = (
        ...
        and latest.close > 0
        and (not strategy["vwap"].get("enabled") or vwap is not None)
    )
    return {
        "ema_fast": ema_fast,
        "ema_slow": ema_slow,
        ...
        "close": latest.close,
        "candle": candle_direction(latest),
        "valid": valid,
    }
...
def apply_signal_to_state(state: BotState, result: SignalResult) -> None:
    ...
    state.current_vwap_side = result.vwap_side
...
def generate_signal(candles: list[Candle], config: dict, state: BotState) -> SignalResult:
    """
    Produce LONG / SHORT / NONE from EMA alignment, session VWAP, and optional filters.
    ...
    Why this function exists:
        A five-second poll can keep seeing the same alignment. The signal
        is a state, not a crossover event. Risk and duplicate-order checks
        decide whether that state becomes an order.
    """
    strategy = config["strategy"]
    ...
    if not indicators["valid"] or ema_fast is None or ema_slow is None or close is None:
        empty.reason = "INDICATORS_INVALID" if candles else "INSUFFICIENT_CANDLES"
        empty.ema_fast = ema_fast
        empty.ema_slow = ema_slow
        empty.vwap = vwap
        empty.close = close
        apply_signal_to_state(state, empty)
        return empty

    trend = "BULLISH" if ema_fast > ema_slow else "BEARISH" if ema_fast < ema_slow else "FLAT"
    vwap_side = "NONE"
    if vwap is not None:
        if close > vwap:
            vwap_side = "ABOVE"
    ...
    volume_needed = float(entry["minimum_volume_multiplier"])
    volume_enabled = volume_needed > 0

    conditions = [
        ConditionResult(
            "EMA",
            bool(entry["require_ema_alignment"]),
            trend in {"BULLISH", "BEARISH"},
            f"fast {ema_fast:.2f} vs slow {ema_slow:.2f} ({trend})",
        ),
        ConditionResult(
            "VWAP",
            bool(entry["require_vwap_confirmation"] and strategy["vwap"].get("enabled", True)),
            vwap is not None and vwap_side in {"ABOVE", "BELOW"},
            f"price {close:.2f} vs VWAP {vwap:.2f}" if vwap is not None else "VWAP unavailable",
        ),
        ConditionResult(
            "Candle",
            bool(entry["candle_confirmation"]),
            candle in {"BULLISH", "BEARISH"},
            candle,
        ),
        ConditionResult(
            "Volume",
            volume_enabled,
            volume_ratio is not None and volume_ratio >= volume_needed,
            f"ratio {volume_ratio:.2f} vs min {volume_needed:.2f}"
    ...
    if entry["require_ema_alignment"]:
        if trend != "BULLISH":
            long_ok = False
            long_reasons.append("EMA_NOT_ALIGNED")
        if trend != "BEARISH":
            short_ok = False
            short_reasons.append("EMA_NOT_ALIGNED")

    if entry["require_vwap_confirmation"] and strategy["vwap"].get("enabled", True):
        if vwap is None:
            long_ok = False
            short_ok = False
            long_reasons.append("VWAP_UNAVAILABLE")
            short_reasons.append("VWAP_UNAVAILABLE")
        else:
            if vwap_side != "ABOVE":
                long_ok = False
                long_reasons.append("PRICE_BELOW_VWAP")
            if vwap_side != "BELOW":
                short_ok = False
                short_reasons.append("PRICE_ABOVE_VWAP")

    if entry["candle_confirmation"]:
        if candle != "BULLISH":
            long_ok = False
            long_reasons.append("CANDLE_NOT_BULLISH")
        if candle != "BEARISH":
            short_ok = False
            short_reasons.append("CANDLE_NOT_BEARISH")

    if volume_enabled:
        if volume_ratio is None or volume_ratio < volume_needed:
            long_ok = False
            short_ok = False
            long_reasons.append("VOLUME_INSUFFICIENT")
            short_reasons.append("VOLUME_INSUFFICIENT")
    ...
    if long_ok and not short_ok:
        ...
        # Alignment should never be both; fail safe.
        signal = "NONE"
        reason = "INDICATORS_INVALID"
        reasons = ["ambiguous LONG and SHORT"]
    else:
        reasons = long_reasons if trend == "BULLISH" else short_reasons if trend == "BEARISH" else ["EMA_NOT_ALIGNED"]
        reason = reasons[0] if reasons else "SCANNING"
    ...
    logging.info("[ANALYSIS] EMA %s=%.2f EMA %s=%.2f VWAP=%s LTP=%.2f",
                 strategy["ema"]["fast_period"],
                 ema_fast,
                 strategy["ema"]["slow_period"],
                 ema_slow,
                 f"{vwap:.2f}" if vwap is not None else "n/a",
                 close)
    logging.info("[SIGNAL] trend=%s vwap=%s candle=%s signal=%s reason=%s",
                 trend, vwap_side, candle, signal, reason)
    return result

def display_signal_reason(result: SignalResult) -> str:
    """Human-readable explanation for the CLI and logs."""
    if result.signal in {"LONG", "SHORT"}:
        return f"Signal: {result.signal} | Trend: {result.trend} | VWAP: {result.vwap_side}"
...
def check_strategy_exits(config: dict, state: BotState, result: SignalResult) -> Optional[str]:
    """
    Exit on opposite EMA+VWAP alignment when configured.

    Opposite-signal exit flattens the position but does not stop the bot.
    TP/SL exits remain the only exits that stop the process.
    """
    if state.current_position == "FLAT":
        return None
    pm = config.get("position_management") or {}
    if not pm.get("exit_on_opposite_signal", True):
        return None
    if state.current_position == "LONG" and result.signal == "SHORT":
        return "OPPOSITE_SIGNAL"
    if state.current_position == "SHORT" and result.signal == "LONG":
        return "OPPOSITE_SIGNAL"
    return None
...
def render_startup_banner(config: dict) -> None:
    ...
    grid.add_row("BOT", str(config["bot"].get("name") or "EMA_VWAP_SCALPER"))
    ...
    grid.add_row("STRATEGY", "EMA + VWAP SCALPER")
    grid.add_row("TIMEFRAME", str(strategy.get("timeframe")))
    grid.add_row("EMA", f"{strategy['ema']['fast_period']} / {strategy['ema']['slow_period']}")
    grid.add_row("ENTRY SIDE", str(config["execution"].get("entry_side") or "LONG"))
...
def render_dashboard(config: dict, state: BotState) -> Panel:
    ...
    system.add_row("STRATEGY", "EMA + VWAP", "TIMEFRAME", str(strategy["timeframe"]))
    ...
    if cli.get("show_indicators", True):
        market_tbl.add_row(
            f"EMA {strategy['ema']['fast_period']}",
            ema_fast_text,
            f"EMA {strategy['ema']['slow_period']}",
            ema_slow_text,
        )
        market_tbl.add_row("TREND", state.current_trend, "VWAP SIDE", state.current_vwap_side)
    ...
    return Panel(
        Group(*sections),
        title="[bold cyan]◈ EMA × VWAP QUANT ENGINE :: DHAN ◈[/bold cyan]",
...
def process_new_candle(
    ...
    if result.signal in {"LONG", "SHORT"}:
        ...
        transaction = "BUY" if result.signal == "LONG" else "SELL"
        state.next_action = f"{result.signal} SIGNAL — EXECUTING"
        if place_entry_order(dhan, config, state, transaction, ltp):
...
def resolve_instrument(config: dict) -> dict:
    ...
    """
    instrument = config["instrument"]
    if not instrument.get("security_id") or not instrument.get("symbol"):
        raise ConfigError("CONFIGURATION ERROR\nsymbol and security_id are required")
    logging.info(
...
def check_stop_signal() -> bool:
    """True when stop.py created .stop or a process signal arrived."""
    return stop_requested()
