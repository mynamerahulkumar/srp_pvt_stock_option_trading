#!/usr/bin/env python3
"""Dhan Price Action Momentum Scalper for NSE equity.
...
``stop.py`` creates ``state/STOP``. This process polls that file and then
shuts down: no new entries, optional close-all, then a clean exit.
...
The strategy evaluates the last completed candle only:

    LONG  = close > prior N-bar high  AND  close > open
            AND body_ratio >= minimum_body_ratio
            + optional volume  + optional trend
    SHORT = close < prior N-bar low   AND  close < open
            AND body_ratio >= minimum_body_ratio
            + optional volume  + optional trend

TP and SL are armed from the actual fill price (percentage or points).
A confirmed TP or SL exit stops the bot for that run by default.
"""
...
WHY_NO_TRADE_LABELS = {
    ...
    "INDICATORS_INVALID": "Price-action data is invalid — no trade",
    "NO_BREAKOUT": "Close has not broken the lookback high or low",
    "BODY_RATIO_LOW": "Candle body is too small versus the range",
    "CANDLE_NOT_BULLISH": "Breakout candle is not bullish",
    "CANDLE_NOT_BEARISH": "Breakout candle is not bearish",
    "VOLUME_INSUFFICIENT": "Volume confirmation failed",
    "TREND_NOT_ALIGNED": "Optional trend confirmation rejected the side",
    "MOVE_TOO_SMALL": "Breakout distance is below the minimum move",
    "LONG_DISABLED": "Long entries are disabled in config",
    "SHORT_DISABLED": "Short entries are disabled in config",
    "SHORT_NOT_SUPPORTED": "Short selling is not supported for this product type",
    ...
    "BOT_DISABLED": "runtime.enabled is false",
    "SCANNING": "Flat — scanning for a price-action breakout",
    ...
    "STATE_UNTRUSTED": "Persisted state could not be trusted — no new entries",
    "ENTRY_EXECUTED": "Entry executed — switching to monitoring",
    "EXIT_EXECUTED": "Exit executed",
    "NONE": "No block",
}

VALID_TP_SL_TYPES = {"PERCENTAGE", "PERCENT", "POINTS", "POINT"}
VALID_ENTRY_SIDES = {"BOTH", "LONG", "SHORT", "BUY", "SELL"}

# Set by signal handlers. Combined with the ``state/STOP`` file.
...
class ConditionResult:
    """One diagnostic strategy check shown on the dashboard."""
...
class SignalResult:
    ...
    signal is LONG, SHORT, or NONE. reason explains why when NONE.
    """
...
@dataclass
class BotState:
    """Runtime state. A JSON snapshot is written so a restart can recover."""
    ...
    loop_cycle: int = 0
    current_ema_fast: Optional[float] = None
    current_ema_slow: Optional[float] = None
    current_trend: str = "NONE"
    current_body_ratio: Optional[float] = None
    current_breakout_high: Optional[float] = None
    current_breakout_low: Optional[float] = None
    current_breakout_percent: Optional[float] = None
...
def parse_cli_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Price Action Momentum Scalper for Dhan")
    parser.add_argument(
        "--config",
...
def _normalize_tp_sl_type(raw: Any) -> str:
...
def _normalize_level_block(parent: dict, name: str, default_value: float) -> dict:
    """Accept either a mapping {enabled, type, value} or a bare number."""
    raw = parent.get(name)
    if isinstance(raw, dict):
        ...
        block = {
            ...
            "type": parent.get(f"{name}_type") or parent.get("tp_sl_mode") or "percentage",
            "value": raw if raw is not None else default_value,
        }
        parent[name] = block
    block["enabled"] = _as_bool(block.get("enabled"), True)
    block["type"] = _normalize_tp_sl_type(block.get("type", "percentage"))
    if block["type"] not in {"PERCENTAGE", "POINTS"}:
        raise ConfigError(
            f"CONFIGURATION ERROR\n{name} type must be percentage or points"
        )
    if block["enabled"]:
        block["value"] = _as_positive_number(block.get("value", default_value), f"risk.{name}.value")
    else:
        block["value"] = _as_non_negative_number(block.get("value", 0), f"risk.{name}.value")
    return block
...
def validate_config(config: dict) -> None:
    ...
    Returns:
        ...
        Dangerous values are never silently replaced with defaults.
    """
    app = config.get("app")
    ...
    if not str(app.get("name") or "").strip():
        app["name"] = "Dhan Price Action Momentum Scalper"
    if not str(app.get("version") or "").strip():
        app["version"] = "1.0.0"
    ...
    schedule = _require_section(config, "schedule")
    emergency = _require_section(config, "emergency_stop")
    cli = _require_section(config, "cli")
    daily_limits = config.get("daily_limits")
    if not isinstance(daily_limits, dict):
        raise ConfigError("CONFIGURATION ERROR\ndaily_limits section is required")
    order_cfg = config.get("order")
    if not isinstance(order_cfg, dict):
        order_cfg = {}
        config["order"] = order_cfg
    logging_cfg = config.get("logging")
    if not isinstance(logging_cfg, dict):
        logging_cfg = {}
        config["logging"] = logging_cfg

    runtime["enabled"] = _as_bool(runtime.get("enabled"), True)
    runtime["dry_run"] = _as_bool(runtime.get("dry_run"), True)
    runtime["allow_live_trading"] = _as_bool(runtime.get("allow_live_trading"), False)
    polling = _as_positive_number(runtime.get("polling_seconds", 5), "runtime.polling_seconds")
    if polling < 1:
        raise ConfigError("CONFIGURATION ERROR\npolling interval must be >= 1")
    runtime["polling_seconds"] = polling
    ...
    runtime["order_wait_seconds"] = _as_positive_number(
        runtime.get("order_wait_seconds", execution.get("order_timeout_seconds", 15)),
        "runtime.order_wait_seconds",
    )
    ...
    runtime["state_file"] = str(runtime.get("state_file") or "state/bot_state.json")
    runtime["name"] = app["name"]

    # Compatibility aliases used by the execution / session / CLI helpers.
    config["bot"] = runtime
    config["trading_session"] = schedule
    config["trading_hours"] = schedule
    config["safety"] = emergency
    ...
    if not security_id:
        raise ConfigError(
            "CONFIGURATION ERROR\ninstrument.security_id is required (example RELIANCE = 2885)"
        )
    if not symbol:
        raise ConfigError("CONFIGURATION ERROR\ninstrument.trading_symbol is required")
    ...
    lookback_candles = int(
        _as_positive_number(strategy.get("lookback_candles", 5), "strategy.lookback_candles")
    )
    strategy["lookback_candles"] = lookback_candles
    strategy["minimum_body_ratio"] = _as_non_negative_number(
        strategy.get("minimum_body_ratio", 0.60), "strategy.minimum_body_ratio"
    )
    if strategy["minimum_body_ratio"] > 1:
        raise ConfigError("CONFIGURATION ERROR\nminimum_body_ratio must be between 0 and 1")
    strategy["minimum_price_move_percent"] = _as_non_negative_number(
        strategy.get("minimum_price_move_percent", 0.0),
        "strategy.minimum_price_move_percent",
    )
    strategy["allow_long"] = _as_bool(strategy.get("allow_long"), True)
    strategy["allow_short"] = _as_bool(strategy.get("allow_short"), True)
    if not strategy["allow_long"] and not strategy["allow_short"]:
        raise ConfigError("CONFIGURATION ERROR\nat least one of allow_long or allow_short must be true")
    strategy["cooldown_seconds"] = _as_non_negative_number(
        strategy.get("cooldown_seconds", 60), "strategy.cooldown_seconds"
    )
    strategy["max_trades_per_day"] = int(
        _as_positive_number(strategy.get("max_trades_per_day", 5), "strategy.max_trades_per_day")
    )

    volume_cfg = strategy.get("volume_confirmation")
    if not isinstance(volume_cfg, dict):
        raise ConfigError("CONFIGURATION ERROR\nstrategy.volume_confirmation is required")
    volume_cfg["enabled"] = _as_bool(volume_cfg.get("enabled"), True)
    volume_cfg["average_period"] = int(
        _as_positive_number(volume_cfg.get("average_period", 20), "strategy.volume_confirmation.average_period")
    )
    volume_cfg["volume_multiplier"] = _as_positive_number(
        volume_cfg.get("volume_multiplier", 1.20),
        "strategy.volume_confirmation.volume_multiplier",
    )

    trend_cfg = strategy.get("trend_confirmation")
    if not isinstance(trend_cfg, dict):
        raise ConfigError("CONFIGURATION ERROR\nstrategy.trend_confirmation is required")
    trend_cfg["enabled"] = _as_bool(trend_cfg.get("enabled"), False)
    trend_cfg["fast_period"] = int(
        _as_positive_number(trend_cfg.get("fast_period", 9), "strategy.trend_confirmation.fast_period")
    )
    trend_cfg["slow_period"] = int(
        _as_positive_number(trend_cfg.get("slow_period", 21), "strategy.trend_confirmation.slow_period")
    )
    if trend_cfg["fast_period"] >= trend_cfg["slow_period"]:
        raise ConfigError("CONFIGURATION ERROR\nfast_period must be less than slow_period")

    needed = lookback_candles + 2
    if volume_cfg["enabled"]:
        needed = max(needed, volume_cfg["average_period"] + 2)
    if trend_cfg["enabled"]:
        needed = max(needed, trend_cfg["slow_period"] + 2)
    if runtime["candle_lookback"] < needed:
        raise ConfigError(
            f"CONFIGURATION ERROR\nruntime.candle_lookback ({runtime['candle_lookback']}) is too small "
            f"for the configured lookback / volume / trend windows (need at least {needed})"
        )

    tp = _normalize_level_block(risk, "take_profit", 0.50)
    sl = _normalize_level_block(risk, "stop_loss", 0.30)
    risk["take_profit_type"] = tp["type"]
    risk["stop_loss_type"] = sl["type"]
    risk["take_profit_value"] = tp["value"]
    risk["stop_loss_value"] = sl["value"]
    risk["take_profit_percent"] = tp["value"] if tp["type"] == "PERCENTAGE" else tp["value"]
    risk["stop_loss_percent"] = sl["value"] if sl["type"] == "PERCENTAGE" else sl["value"]

    trail = risk.get("trailing_stop")
    if not isinstance(trail, dict):
        trail = {"enabled": False, "activation_percent": 0.50, "trail_percent": 0.25}
    ...
    trail["trail_percent"] = _as_non_negative_number(
        trail.get("trail_percent", trail.get("distance_percent", 0.25)),
        "risk.trailing_stop.trail_percent",
    )
    ...
    daily_limits["enabled"] = _as_bool(daily_limits.get("enabled"), True)
    daily_limits["max_loss"] = _as_non_negative_number(
        daily_limits.get("max_loss", 0), "daily_limits.max_loss"
    )
    daily_limits["max_profit"] = _as_non_negative_number(
        daily_limits.get("max_profit", 0), "daily_limits.max_profit"
    )
    daily_limits["close_position_on_limit"] = _as_bool(
        daily_limits.get("close_position_on_limit"), True
    )
    risk["max_daily_loss"] = daily_limits["max_loss"] if daily_limits["enabled"] else 0
    risk["max_daily_profit"] = daily_limits["max_profit"] if daily_limits["enabled"] else 0
    risk["close_on_risk_limit"] = daily_limits["close_position_on_limit"]
    risk["max_trades_per_day"] = strategy["max_trades_per_day"]
    risk["max_daily_trades"] = strategy["max_trades_per_day"]
    risk["max_open_positions"] = 1 if execution.get("one_position_at_a_time", True) else 1
    ...
    risk["stop_bot_after_tp_sl"] = risk["stop_bot_after_exit"]

    order_type = str(order_cfg.get("order_type") or execution.get("order_type") or "MARKET").strip().upper()
    if order_type not in VALID_ORDER_TYPES:
        raise ConfigError("CONFIGURATION ERROR\norder type must be MARKET or LIMIT")
    order_cfg["order_type"] = order_type
    order_cfg["validity"] = str(order_cfg.get("validity") or "DAY").strip().upper() or "DAY"
    order_cfg["disclosed_quantity"] = int(
        _as_non_negative_number(order_cfg.get("disclosed_quantity", 0), "order.disclosed_quantity")
    )
    order_cfg["amo"] = _as_bool(order_cfg.get("amo"), False)
    execution["order_type"] = order_type
    execution["validity"] = order_cfg["validity"]
    execution["disclosed_quantity"] = order_cfg["disclosed_quantity"]
    execution["after_market_order"] = order_cfg["amo"]
    execution["enable_long"] = strategy["allow_long"]
    execution["enable_short"] = strategy["allow_short"]
    execution["allow_long"] = strategy["allow_long"]
    execution["allow_short"] = strategy["allow_short"]
    if strategy["allow_long"] and strategy["allow_short"]:
        execution["entry_side"] = "BOTH"
    elif strategy["allow_short"]:
        execution["entry_side"] = "SHORT"
    else:
        execution["entry_side"] = "LONG"
    if execution["enable_short"] and product_type in {"CNC", "MTF"}:
        raise ConfigError(
            "CONFIGURATION ERROR\nshort selling is not supported for CNC or MTF"
        )
    execution["one_position_at_a_time"] = _as_bool(
        execution.get("one_position_at_a_time"), True
    )
    execution["verify_order_status"] = _as_bool(execution.get("verify_order_status"), True)
    execution["verify_position_before_entry"] = _as_bool(
        execution.get("verify_position_before_entry"), True
    )
    execution["max_entry_slippage_percent"] = _as_non_negative_number(
        execution.get("max_entry_slippage_percent", 0.10),
        "execution.max_entry_slippage_percent",
    )
    execution["max_exit_slippage_percent"] = _as_non_negative_number(
        execution.get("max_exit_slippage_percent", 0.20),
        "execution.max_exit_slippage_percent",
    )
    execution["order_tag"] = str(execution.get("order_tag") or "PAM_SCALPER").strip() or "PAM_SCALPER"
    execution["wait_for_order_confirmation"] = _as_bool(
        execution.get("wait_for_order_confirmation"), True
    )
    execution["order_status_poll_seconds"] = _as_positive_number(
        execution.get("order_status_poll_seconds", 1),
        "execution.order_status_poll_seconds",
    )
    execution["order_timeout_seconds"] = _as_positive_number(
        execution.get("order_timeout_seconds", runtime["order_wait_seconds"]),
        "execution.order_timeout_seconds",
    )
    runtime["order_wait_seconds"] = execution["order_timeout_seconds"]
    execution["prevent_duplicate_orders"] = _as_bool(
        execution.get("prevent_duplicate_orders"), True
    )
    execution["cooldown_seconds"] = strategy["cooldown_seconds"]
    limit_mode = str(
        order_cfg.get("limit_price_mode") or execution.get("limit_price_mode") or "OFFSET_PERCENT"
    ).strip().upper()
    ...
    execution["limit_offset_percent"] = _as_non_negative_number(
        order_cfg.get("limit_offset_percent", execution.get("limit_offset_percent", 0)),
        "order.limit_offset_percent",
    )
    execution["limit_price"] = _as_non_negative_number(
        order_cfg.get("limit_price", execution.get("limit_price", 0)),
        "order.limit_price",
    )
    ...
    schedule["enabled"] = _as_bool(schedule.get("enabled"), False)
    schedule["timezone"] = str(schedule.get("timezone") or "Asia/Kolkata").strip()
    schedule["close_all_positions_at_stop_time"] = _as_bool(
        schedule.get("close_all_positions_at_stop_time"), True
    )
    stop_time = str(schedule.get("stop_time") or "15:15")
    schedule["stop_time"] = stop_time
    schedule["stop_entry_time"] = stop_time
    schedule["stop_new_entries_time"] = stop_time
    schedule["close_positions_time"] = stop_time
    schedule["close_all_positions_time"] = stop_time
    start_h, start_m = _parse_hhmm(schedule.get("start_time", "09:20"), "schedule.start_time")
    stop_h, stop_m = _parse_hhmm(stop_time, "schedule.stop_time")
    if schedule["enabled"] and (start_h, start_m) >= (stop_h, stop_m):
    ...
    startup["behavior"] = "WAIT_FOR_SIGNAL"
    ...
    position_management["monitor_pnl"] = _as_bool(
        position_management.get("monitor_pnl"), True
    )
    position_management["exit_on_tp"] = _as_bool(position_management.get("exit_on_tp"), True)
    position_management["exit_on_sl"] = _as_bool(position_management.get("exit_on_sl"), True)
    position_management["exit_on_opposite_signal"] = False
    position_management["manage_only_bot_positions"] = _as_bool(
        position_management.get("manage_only_bot_positions"), True
    )
    strategy["exit"] = {"opposite_signal": False}

    emergency["close_position"] = _as_bool(emergency.get("close_position"), True)
    emergency["cancel_pending_orders_on_shutdown"] = _as_bool(
        emergency.get("cancel_pending_orders_on_shutdown"), True
    )
    emergency["close_positions_on_error"] = _as_bool(
        emergency.get("close_positions_on_error"), False
    )
    emergency["max_api_retries"] = int(
        _as_positive_number(emergency.get("max_api_retries", 3), "emergency_stop.max_api_retries")
    )
    emergency["retry_delay_seconds"] = _as_positive_number(
        emergency.get("retry_delay_seconds", 2), "emergency_stop.retry_delay_seconds"
    )
    emergency["manage_only_configured_symbol"] = _as_bool(
        emergency.get("manage_only_configured_symbol"), True
    )
    emergency["require_flat_before_entry"] = _as_bool(
        emergency.get("require_flat_before_entry"), True
    )
    emergency["block_if_unexpected_position"] = _as_bool(
        emergency.get("block_if_unexpected_position"), True
    )
    emergency["require_confirmation_for_live_trading"] = _as_bool(
        emergency.get("require_confirmation_for_live_trading"), False
    )
    emergency["close_positions_on_shutdown"] = emergency["close_position"]
    emergency["prevent_duplicate_orders"] = True

    shutdown = {
        "close_positions_on_manual_stop": emergency["close_position"],
        "close_positions_on_session_end": schedule["close_all_positions_at_stop_time"],
        "close_positions_on_tp_sl": True,
        "close_positions_on_error": emergency["close_positions_on_error"],
        "cancel_pending_orders": emergency["cancel_pending_orders_on_shutdown"],
    }
    config["shutdown"] = shutdown

    logging_cfg["level"] = str(logging_cfg.get("level") or "INFO").strip().upper()
    logging_cfg["file"] = str(logging_cfg.get("file") or "logs/price_action_momentum.log")
    logging_cfg["max_bytes"] = int(
        _as_positive_number(logging_cfg.get("max_bytes", 5242880), "logging.max_bytes")
    )
    logging_cfg["backup_count"] = int(
        _as_non_negative_number(logging_cfg.get("backup_count", 3), "logging.backup_count")
    )
    ...
    cli["show_signal_details"] = _as_bool(cli.get("show_signal_details"), True)
    cli["show_recent_logs"] = _as_bool(cli.get("show_recent_logs"), True)
    cli["show_signal_status"] = _as_bool(
        cli.get("show_signal_status", cli.get("show_signal_details")), True
    )
    cli["show_position_pnl"] = _as_bool(cli.get("show_position_pnl"), True)
    cli["show_indicators"] = _as_bool(cli.get("show_indicators", cli.get("show_signal_details")), True)
    cli["show_signal"] = _as_bool(cli.get("show_signal", cli.get("show_signal_details")), True)
...
def is_live_mode(config: dict) -> bool:
    runtime = config.get("runtime") or config.get("bot") or {}
    return (not bool(runtime.get("dry_run", True))) and bool(
        runtime.get("allow_live_trading", False)
    )
...
def validate_safety_config(config: dict) -> None:
    ...
    if dry_run:
        return
    if not runtime.get("allow_live_trading"):
        raise SafetyError(
            "ERROR: Live trading is not fully unlocked.\n"
            "Set runtime.dry_run: false and runtime.allow_live_trading: true.\n"
            "The bot did not start."
        )
...
def setup_logging(config: dict) -> None:
    """Console + rotating file. Tokens and client IDs are never written."""
    ...
    log_path = Path(str(logging_cfg.get("file") or "logs/price_action_momentum.log"))
    ...
    if not any(isinstance(handler, RotatingFileHandler) for handler in root.handlers):
        file_handler = RotatingFileHandler(
            log_path,
            maxBytes=int(logging_cfg.get("max_bytes") or 5242880),
            backupCount=int(logging_cfg.get("backup_count") or 3),
            encoding="utf-8",
        )
...
def _state_file_path(config: dict) -> Path:
    raw = str((config.get("runtime") or {}).get("state_file") or STATE_FILE)
    path = Path(raw)
...
def persist_state(config: dict, state: BotState) -> None:
    """
    Write a lightweight JSON snapshot. Secrets are never stored.

    Why this function exists:
        A restart during the same session must remember trade count,
        last processed candle, pending order IDs, and armed TP/SL so
        the bot does not open a duplicate entry.
    """
    if state.state_untrusted:
        return
    path = _state_file_path(config)
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "bot_status": state.engine_state,
        "trading_day": state.trading_day,
        "trades_today": state.trades_today,
        "daily_realized_pnl": state.daily_realized_pnl,
        "last_processed_candle": _dt_to_iso(state.last_processed_candle),
        "last_signal": state.last_signal,
        "entry_order_id": state.pending_order_id or state.last_order_id,
        "pending_order_id": state.pending_order_id,
        "pending_order_tag": state.pending_order_tag,
        "pending_order_intent": state.pending_order_intent,
        "pending_order_side": state.pending_order_side,
        "pending_order_quantity": state.pending_order_quantity,
        "order_in_flight": state.order_in_flight,
        "entry_price": state.entry_price,
        "take_profit_price": state.take_profit_price,
        "stop_loss_price": state.stop_loss_price,
        "position_direction": state.current_position,
        "position_quantity": state.position_quantity,
        "last_trade_time": _dt_to_iso(state.last_trade_time),
        "last_exit_reason": state.last_exit_reason,
        "traded_candle": _dt_to_iso(state.extra.get("traded_candle")),
    }
    ...
    try:
        ...
        logging.warning("Could not persist bot state: %s", exc)


def load_persisted_state(config: dict, state: BotState) -> None:
    """
    Restore a previous snapshot. A corrupt file fails safe: no new entries.

    Broker positions always win over local fields during recovery.
    """
    path = _state_file_path(config)
    if not path.exists():
        return
    try:
        ...
        logging.error("State file is untrusted (%s). New entries are blocked.", exc)
        state.state_untrusted = True
        state.why_no_trade = "STATE_UNTRUSTED"
        return

    today = now_in_session_tz(config).date().isoformat()
    stored_day = str(payload.get("trading_day") or "")
    state.trading_day = stored_day or today
    if stored_day and stored_day != today:
        logging.info("Persisted state is from %s. Resetting daily counters for %s.", stored_day, today)
    ...
    state.trades_today = _safe_int(payload.get("trades_today"), 0)
    state.daily_realized_pnl = _safe_float(payload.get("daily_realized_pnl"), 0.0)
    state.last_processed_candle = _iso_to_dt(payload.get("last_processed_candle"))
    state.last_signal = str(payload.get("last_signal") or "NONE")
    state.pending_order_id = payload.get("pending_order_id") or None
    state.pending_order_tag = payload.get("pending_order_tag") or None
    state.pending_order_intent = payload.get("pending_order_intent") or None
    state.pending_order_side = payload.get("pending_order_side") or None
    state.pending_order_quantity = _safe_int(payload.get("pending_order_quantity"), 0)
    state.order_in_flight = bool(payload.get("order_in_flight")) and bool(state.pending_order_id)
    if payload.get("entry_price"):
        state.entry_price = _safe_float(payload.get("entry_price"), 0.0) or None
    if payload.get("take_profit_price"):
        state.take_profit_price = _safe_float(payload.get("take_profit_price"), 0.0) or None
    if payload.get("stop_loss_price"):
        state.stop_loss_price = _safe_float(payload.get("stop_loss_price"), 0.0) or None
    if payload.get("position_direction") in {"LONG", "SHORT"}:
        state.current_position = str(payload["position_direction"])
        state.position_quantity = _safe_int(payload.get("position_quantity"), 0)
        state.trade_direction = state.current_position
    state.last_trade_time = _iso_to_dt(payload.get("last_trade_time"))
    state.last_exit_reason = str(payload.get("last_exit_reason") or "—")
    traded = _iso_to_dt(payload.get("traded_candle"))
    if traded is not None:
        state.extra["traded_candle"] = traded
    logging.info(
        "Loaded persisted state for %s: trades_today=%s pending=%s position=%s",
        state.trading_day,
        state.trades_today,
        state.pending_order_id or "none",
...
def consume_leftover_stop_file() -> None:
    """Delete a leftover state/STOP from a previous run so a new start is not halted."""
...
def session_timezone(config: dict):
    hours = config.get("trading_hours") or config.get("trading_session") or {}
    name = str(hours.get("timezone") or "Asia/Kolkata")
...
def validate_candles(candles: list[Candle]) -> list[Candle]:
    ...
    Why this function exists:
        A bad OHLC row can invent a fake breakout. Unreliable data
...
def get_market_data(dhan, config: dict) -> list[Candle]:
    """
    Fetch required candle history for breakout, body, volume, and trend.
...
def calculate_candle_metrics(candle: Candle) -> dict[str, Any]:
    """
    Body size, range, and body ratio for one completed candle.

    Use case:
        A breakout candle with a large body relative to its range is more
        likely to show directional follow-through than a long-wick doji.

    Returns:
        body_size, candle_range, body_ratio, direction. body_ratio is 0
        when the range is zero so a flat bar cannot pass a body filter.
    """
...
def calculate_breakout_levels(
    ...
    Use case:
        The strategy uses these levels to determine whether price has
        broken out of its recent trading range.

    Inputs:
        Completed candles, oldest-first. The last bar is the evaluation
        candle and is excluded so we never use its own high/low as the
        level it is supposed to break.
...
def calculate_volume_confirmation(
    ...
    """
    Compare the evaluation candle's volume to the prior average.

    The evaluation bar is excluded from the average so a spike is not
    compared against itself.
    """
    if average_period <= 0 or len(candles) < average_period + 1:
        return {
    ...
    window = [item.volume for item in candles[-(average_period + 1) : -1]]
    ...
    return {
        ...
        "average_volume": average,
        "volume_ratio": ratio,
        "passed": current >= average * volume_multiplier,
    }


def calculate_trend_confirmation(
    candles: list[Candle],
    fast_period: int,
    slow_period: int,
) -> dict[str, Any]:
    """
    Optional fast/slow EMA relationship on completed closes.

    LONG confirmation: fast > slow. SHORT confirmation: fast < slow.
    Disabled filters should not be called; the values are still useful
    on the dashboard when the filter is on.
    """
    closes = [item.close for item in candles]
    fast_series = calculate_ema(closes, fast_period)
    slow_series = calculate_ema(closes, slow_period)
    ema_fast = fast_series[-1] if fast_series else None
    ema_slow = slow_series[-1] if slow_series else None
    if ema_fast is None or ema_slow is None:
        ...
        trend = "FLAT"
    return {"ema_fast": ema_fast, "ema_slow": ema_slow, "trend": trend}


def candle_direction(candle: Candle) -> str:
    """Bullish when close > open, bearish when close < open, else doji."""
    if candle.close > candle.open:
        return "BULLISH"
    if candle.close < candle.open:
        return "BEARISH"
...
def check_long_signal(
    ...
    """
    Validate a long setup. Pure function — no Dhan calls.

    Required: close > breakout_high, close > open, body_ratio threshold.
    Optional: volume, trend, minimum price-move percent.
    """
    reasons: list[str] = []
    strategy = config["strategy"]
    close = metrics["close"]
    ...
    if metrics["direction"] != "BULLISH":
        reasons.append("CANDLE_NOT_BULLISH")
    if metrics["body_ratio"] < float(strategy["minimum_body_ratio"]):
        reasons.append("BODY_RATIO_LOW")
    min_move = float(strategy["minimum_price_move_percent"])
    if min_move > 0 and breakout_high > 0:
        move = ((close - breakout_high) / breakout_high) * 100.0
        if move < min_move:
            reasons.append("MOVE_TOO_SMALL")
    if strategy["volume_confirmation"]["enabled"] and not volume_info.get("passed"):
        reasons.append("VOLUME_INSUFFICIENT")
    if strategy["trend_confirmation"]["enabled"] and trend_info.get("trend") != "BULLISH":
        reasons.append("TREND_NOT_ALIGNED")
    if not strategy.get("allow_long", True):
...
def check_short_signal(
    ...
    """Validate a short setup. Mirror of check_long_signal."""
    reasons: list[str] = []
    strategy = config["strategy"]
    close = metrics["close"]
    ...
    if close >= breakout_low:
        reasons.append("NO_BREAKOUT")
    if metrics["direction"] != "BEARISH":
        reasons.append("CANDLE_NOT_BEARISH")
    if metrics["body_ratio"] < float(strategy["minimum_body_ratio"]):
        reasons.append("BODY_RATIO_LOW")
    min_move = float(strategy["minimum_price_move_percent"])
    if min_move > 0 and breakout_low > 0:
        move = ((breakout_low - close) / breakout_low) * 100.0
        if move < min_move:
            reasons.append("MOVE_TOO_SMALL")
    if strategy["volume_confirmation"]["enabled"] and not volume_info.get("passed"):
        reasons.append("VOLUME_INSUFFICIENT")
    if strategy["trend_confirmation"]["enabled"] and trend_info.get("trend") != "BEARISH":
        reasons.append("TREND_NOT_ALIGNED")
    if not strategy.get("allow_short", True):
        reasons.append("SHORT_DISABLED")
    return (len(reasons) == 0), reasons


def apply_signal_to_state(state: BotState, result: SignalResult) -> None:
    """Copy pure strategy output into the dashboard/runtime state."""
    state.last_signal = result.signal
    state.last_signal_time = datetime.now(timezone.utc)
    state.current_ema_fast = result.ema_fast
    state.current_ema_slow = result.ema_slow
    state.current_trend = result.trend
    state.current_body_ratio = result.body_ratio
    state.current_breakout_high = result.breakout_high
    state.current_breakout_low = result.breakout_low
    state.current_breakout_percent = result.breakout_percent
    ...
    state.signal_reasons = list(result.reasons)
    state.confirmation_status = result.signal
...
def generate_signal(candles: list[Candle], config: dict, state: BotState) -> SignalResult:
    """
    Produce LONG / SHORT / NONE from completed-candle price-action rules.

    Parameters:
        candles: completed bars only. The forming candle is never used.
        config: validated YAML.
        state: runtime state updated for the dashboard.

    Returns:
        SignalResult. This function never talks to Dhan and never places an order.

    Look-ahead rule:
        The last bar is the evaluation candle. Breakout levels, volume
        average, and trend EMAs use strictly earlier information where
    ...
    strategy = config["strategy"]
    lookback = int(strategy["lookback_candles"])
    if len(candles) < lookback + 1:
        empty.reason = "INSUFFICIENT_CANDLES"
        apply_signal_to_state(state, empty)
        return empty

    eval_candle = candles[-1]
    metrics = calculate_candle_metrics(eval_candle)
    breakout_high, breakout_low = calculate_breakout_levels(candles, lookback)
    volume_info = calculate_volume_confirmation(
        candles,
        int(strategy["volume_confirmation"]["average_period"]),
        float(strategy["volume_confirmation"]["volume_multiplier"]),
    )
    trend_info = calculate_trend_confirmation(
        candles,
        int(strategy["trend_confirmation"]["fast_period"]),
        int(strategy["trend_confirmation"]["slow_period"]),
    )
    if breakout_high is None or breakout_low is None:
        empty.reason = "INSUFFICIENT_CANDLES"
        empty.close = metrics["close"]
        apply_signal_to_state(state, empty)
        return empty

    long_ok, long_reasons = check_long_signal(
        metrics, breakout_high, config, volume_info, trend_info
    )
    short_ok, short_reasons = check_short_signal(
        metrics, breakout_low, config, volume_info, trend_info
    )

    breakout_percent = None
    if eval_candle.close > breakout_high > 0:
        breakout_percent = ((eval_candle.close - breakout_high) / breakout_high) * 100.0
    elif eval_candle.close < breakout_low > 0:
        breakout_percent = ((breakout_low - eval_candle.close) / breakout_low) * 100.0

    conditions = [
        ConditionResult(
            "Breakout",
            True,
            (long_ok and "NO_BREAKOUT" not in long_reasons)
            or (short_ok and "NO_BREAKOUT" not in short_reasons)
            or eval_candle.close > breakout_high
            or eval_candle.close < breakout_low,
            f"high {breakout_high:.2f} / low {breakout_low:.2f} close {eval_candle.close:.2f}",
        ),
        ConditionResult(
            "Body",
            True,
            metrics["body_ratio"] >= float(strategy["minimum_body_ratio"]),
            f"ratio {metrics['body_ratio']:.2f} vs min {strategy['minimum_body_ratio']:.2f}",
        ),
        ConditionResult(
            "Volume",
            bool(strategy["volume_confirmation"]["enabled"]),
            bool(volume_info.get("passed")),
            f"ratio {volume_info['volume_ratio']:.2f} vs x{strategy['volume_confirmation']['volume_multiplier']:.2f}"
            if volume_info.get("volume_ratio") is not None
            else "volume average unavailable",
        ),
        ConditionResult(
            "Trend",
            bool(strategy["trend_confirmation"]["enabled"]),
            trend_info.get("trend") in {"BULLISH", "BEARISH"},
            f"{trend_info.get('trend')} fast={trend_info.get('ema_fast')} slow={trend_info.get('ema_slow')}",
        ),
    ]
    ...
    if long_ok and not short_ok:
        ...
        reasons = ["ambiguous LONG and SHORT"]
    else:
        reasons = long_reasons if eval_candle.close >= eval_candle.open else short_reasons
        if not reasons:
            reasons = ["NO_BREAKOUT"]
        reason = reasons[0]
    ...
    result = SignalResult(
        signal=signal,
        reason=reason,
        trend=str(trend_info.get("trend") or "NONE"),
        candle=metrics["direction"],
        close=metrics["close"],
        open=metrics["open"],
        high=metrics["high"],
        low=metrics["low"],
        ...
        volume=volume_info.get("current_volume"),
        average_volume=volume_info.get("average_volume"),
    ...
    logging.info(
        "[ANALYSIS] close=%.2f high=%.2f low=%.2f body=%.2f brk_hi=%.2f brk_lo=%.2f",
        metrics["close"],
        metrics["high"],
        metrics["low"],
        metrics["body_ratio"],
    ...
    logging.info("[SIGNAL] candle=%s trend=%s signal=%s reason=%s",
                 metrics["direction"], result.trend, signal, reason)
    return result


def display_signal_reason(result: SignalResult) -> str:
    """Human-readable explanation for the CLI and logs."""
    if result.signal in {"LONG", "SHORT"}:
        body = f"{result.body_ratio:.2f}" if result.body_ratio is not None else "n/a"
        return f"Signal: {result.signal} | Body: {body} | Trend: {result.trend}"
    label = WHY_NO_TRADE_LABELS.get(result.reason, result.reason)
    return f"Signal: NONE | Reason: {label}"
...
def _offset_price(entry_price: float, value: float, level_type: str, favorable: bool) -> float:
    """Move ``value`` percent or points away from entry in the requested direction."""
    if level_type == "POINTS":
        delta = value
    else:
        delta = entry_price * (value / 100.0)
    return entry_price + delta if favorable else entry_price - delta


def calculate_sl_price(entry_price: float, side: str, config: dict) -> Optional[float]:
    """
    Stop-loss from the actual fill price.

    Parameters:
        entry_price: broker fill, not the requested order price.
        side: LONG or SHORT.
        config: risk.stop_loss {enabled, type, value}.

    Returns:
        Tick-rounded SL, or None when SL is disabled.

    LONG percentage:  E * (1 - SL% / 100)
    SHORT percentage: E * (1 + SL% / 100)
    LONG points:      E - points
    SHORT points:     E + points
    """
    ...
    raw = _offset_price(
        ...
        float(block["value"]),
        str(block["type"]),
        favorable=(side.upper() == "SHORT"),
    )
    return round_to_tick(raw, float(config["instrument"]["tick_size"]))


def calculate_tp_price(entry_price: float, side: str, config: dict) -> Optional[float]:
    """
    Take-profit from the actual fill price.

    We use the actual broker fill rather than the requested order price
    because slippage can cause the execution price to differ.
    """
    ...
    raw = _offset_price(
        ...
        float(block["value"]),
        str(block["type"]),
        favorable=(side.upper() == "LONG"),
    )
    return round_to_tick(raw, float(config["instrument"]["tick_size"]))


def calculate_stop_loss(entry_price: float, side: str, percent: float) -> float:
    """Legacy percent helper kept for readability in comments and tests."""
    if side.upper() == "LONG":
...
def calculate_exit_levels(
    ...
    """Reusable TP/SL pair rounded to tick size. Uses fill price as the reference."""
    return calculate_sl_price(entry_price, side, config), calculate_tp_price(
...
def arm_risk_levels(state: BotState, config: dict, entry_price: float, side: str) -> None:
    ...
    update_trailing_stop(state, config)
    logging.info("TP = %s", f"{tp:.2f}" if tp is not None else "off")
    logging.info("SL = %s", f"{sl:.2f}" if sl is not None else "off")
    logging.info(
        "TP ARMED / SL ARMED | entry=%.2f side=%s TP=%s SL=%s",
        entry_price,
        side,
        f"{tp:.2f}" if tp is not None else "off",
        f"{sl:.2f}" if sl is not None else "off",
    )
...
def _apply_fill(
    ...
    state.last_entry_signal = position_side
    ...
    if price > 0:
        ...
        reference = state.last_ltp or price
        if reference > 0:
            slip_pct = abs(price - reference) / reference * 100.0
            limit = float(config["execution"].get("max_entry_slippage_percent") or 0)
            if limit > 0 and slip_pct > limit:
                logging.warning(
                    "Entry slippage %.3f%% exceeds max_entry_slippage_percent %.3f%% "
                    "(fill=%.2f reference=%.2f). Position is kept; no extra order.",
                    slip_pct,
                    limit,
                    price,
                    reference,
                ...
                logging.info("Entry slippage versus LTP: %.3f%%", slip_pct)
...
def build_order_request(
    ...
    return {
        ...
        "price": float(price),
        "trigger_price": 0,
        "disclosed_quantity": int(execution.get("disclosed_quantity") or 0),
        "after_market_order": bool(execution.get("after_market_order", False)),
        "validity": _map_validity(execution.get("validity", "DAY")),
        "tag": tag or str(execution.get("order_tag") or "PAM_SCALPER"),
    }
...
def submit_order(
    ...
    if is_dry_run(config):
        ...
        if intent == "ENTRY" and fill_price > 0:
            side = "LONG" if transaction_type == "BUY" else "SHORT"
            tp = calculate_tp_price(fill_price, side, config)
            sl = calculate_sl_price(fill_price, side, config)
            logging.info(
                "[PAPER ORDER] %s %s %s | Entry: %s | TP: %s | SL: %s",
                side,
                quantity,
                config["instrument"]["symbol"],
                format_inr(fill_price),
                format_inr(tp),
                format_inr(sl),
            )
    ...
    tag = f"{str(config['execution'].get('order_tag') or 'PAM')[:8]}{uuid.uuid4().hex[:8]}"
...
def wait_for_order_completion(
    ...
    poll_interval = float(
        config["execution"].get("order_status_poll_seconds")
        or config["bot"]["polling_seconds"]
    )
    max_wait = float(
        config["execution"].get("order_timeout_seconds")
        or config["bot"]["order_wait_seconds"]
    )
...
def recover_existing_state(dhan, config: dict, state: BotState) -> None:
    ...
    if avg <= 0:
        ...
        state.why_no_trade = "BLOCKED_UNEXPECTED_POSITION"
        return
    logging.info("RECOVERING EXISTING POSITION")
...
def validate_trade(
    ...
    if config["execution"].get("prevent_duplicate_orders", True) or config["safety"].get(
    ...
    if side == "LONG" and not config["execution"].get("enable_long"):
        return False, "LONG_DISABLED"
    if side == "SHORT" and not config["execution"].get("enable_short"):
        return False, "SHORT_DISABLED"
    if side == "SHORT" and str(config["instrument"]["product_type"]).upper() in {"CNC", "MTF"}:
        return False, "SHORT_NOT_SUPPORTED"
...
def compute_next_action(config: dict, state: BotState) -> None:
    ...
    if state.why_no_trade in blocking:
        ...
        return
    if state.last_signal in {"LONG", "SHORT"} and state.why_no_trade in {
        "NONE",
        "SCANNING",
        "ENTRY_EXECUTED",
    }:
        state.next_action = f"{state.last_signal} SIGNAL — RISK CHECK PENDING"
        return
...
def monitor_tpsl(dhan, config: dict, state: BotState, ltp: Optional[float]) -> Optional[str]:
    ...
    pm = config.get("position_management") or {}
    if should_trigger_sl(ltp, stop, side) and pm.get("exit_on_sl", True):
        ...
        return "STOP_LOSS"
    if should_trigger_tp(ltp, state.take_profit_price, side) and pm.get("exit_on_tp", True):
        return "TAKE_PROFIT"
    return None
...
def check_strategy_exits(config: dict, state: BotState, result: SignalResult) -> Optional[str]:
    """
    Price-action exits are TP/SL only. Opposite-signal flatten is disabled.

    Kept as a named function so the poll loop stays readable.
    """
    return None
...
def render_startup_banner(config: dict) -> None:
    ...
    grid.add_row("BOT", str(config["app"].get("name") or "Price Action Momentum Scalper"))
    grid.add_row("MODE", "DRY RUN" if is_dry_run(config) else "LIVE")
    grid.add_row("SYMBOL", str(instrument.get("trading_symbol") or instrument.get("symbol")))
    grid.add_row("SECURITY ID", str(instrument.get("security_id")))
    grid.add_row("STRATEGY", "PRICE ACTION MOMENTUM")
    grid.add_row("TIMEFRAME", str(strategy.get("timeframe")))
    grid.add_row("LOOKBACK", str(strategy.get("lookback_candles")))
    grid.add_row("BODY RATIO", str(strategy.get("minimum_body_ratio")))
    grid.add_row("ENTRY SIDE", str(config["execution"].get("entry_side") or "BOTH"))
    grid.add_row("SL", f"{sl['value']} {sl['type'].lower()}" if sl.get("enabled") else "off")
    grid.add_row("TP", f"{tp['value']} {tp['type'].lower()}" if tp.get("enabled") else "off")
...
def render_exit_banner(
    ...
    titles = {
        "TAKE_PROFIT": "TP HIT → EXIT EXECUTED → BOT STOPPED",
        "STOP_LOSS": "SL HIT → EXIT EXECUTED → BOT STOPPED",
        "TRAILING_STOP": "TRAILING STOP → EXIT EXECUTED → BOT STOPPED",
        "OPPOSITE_SIGNAL": "OPPOSITE SIGNAL → EXIT EXECUTED",
        "TRADING_DAY_END": "DAY SESSION CLOSE",
    ...
    if reason == "TRADING_DAY_END":
        grid.add_row("SESSION", "NEW ENTRIES DISABLED → POSITIONS FLAT → BOT STOPPED")
...
def render_dashboard(config: dict, state: BotState) -> Panel:
    ...
    ema_fast_text = (
        f"{state.current_ema_fast:.2f}" if state.current_ema_fast is not None else "—"
    )
    ema_slow_text = (
        f"{state.current_ema_slow:.2f}" if state.current_ema_slow is not None else "—"
    )
    ...
    signal_icon = {"LONG": "▲ LONG", "SHORT": "▼ SHORT"}.get(state.last_signal, "NONE")
    ...
    system.add_row("STRATEGY", "PRICE ACTION MOMENTUM", "TIMEFRAME", str(strategy["timeframe"]))
    ...
    market_tbl.add_column(min_width=20)
    brk_hi = (
        format_inr(state.current_breakout_high) if state.current_breakout_high is not None else "—"
    )
    brk_lo = (
        format_inr(state.current_breakout_low) if state.current_breakout_low is not None else "—"
    )
    body_text = (
        f"{state.current_body_ratio:.2f}" if state.current_body_ratio is not None else "—"
    )
    market_tbl.add_row("LTP", format_inr(state.last_ltp), "BODY RATIO", body_text)
    if cli.get("show_indicators", True):
        market_tbl.add_row("BRK HIGH", brk_hi, "BRK LOW", brk_lo)
        if strategy["trend_confirmation"]["enabled"]:
            market_tbl.add_row(
                f"EMA {strategy['trend_confirmation']['fast_period']}",
                ema_fast_text,
                f"EMA {strategy['trend_confirmation']['slow_period']}",
                ema_slow_text,
            )
            market_tbl.add_row("TREND", state.current_trend, "LOOKBACK", str(strategy["lookback_candles"]))
        else:
            market_tbl.add_row("TREND", "OFF", "LOOKBACK", str(strategy["lookback_candles"]))
    ...
    if cli.get("show_signal", True):
        ...
        strat.add_row("SIGNAL", signal_icon)
        strat.add_row("CHECKS", ticks)
        if state.signal_reasons:
            strat.add_row("REASONS", " | ".join(state.signal_reasons))
    ...
    return Panel(
        Group(*sections),
        title="[bold cyan]◈ DHAN // PRICE ACTION MOMENTUM CORE ◈[/bold cyan]",
...
def process_new_candle(
    ...
    if state.current_position != "FLAT":
        ...
        if strategy_exit:
            logging.info("[EXIT] %s", strategy_exit.replace("_", " "))
            ...
            # Opposite-signal exit does not stop the bot. TP/SL does.
    ...
    if result.signal in {"LONG", "SHORT"}:
        state.engine_state = "WAITING_FOR_SIGNAL"
        logging.info("[SIGNAL] %s alignment detected", result.signal)
        push_event("SIGNAL", display_signal_reason(result))
        allowed, reason = validate_trade(config, state, result.signal)
        if not allowed:
            logging.info("[RISK] Trade blocked: %s", WHY_NO_TRADE_LABELS.get(reason, reason))
            state.why_no_trade = reason
            compute_next_action(config, state)
            return None
        logging.info("[RISK] Risk check passed")
        if is_live_mode(config):
            logging.info("LIVE TRADING ENABLED")
            logging.info(
                "LIVE pre-trade chain: data → indicators → signal → session → "
                "risk → duplicate check → order → fill"
            )
        state.next_action = f"{result.signal} SIGNAL — EXECUTING"
        if execute_signal(dhan, config, state, result.signal, ltp):
...
def resolve_instrument(config: dict) -> dict:
    """
    Validate the configured instrument. Security IDs come from YAML.

    Why this function exists:
        Downloading the full security master would waste RAM on a 1 GB VM.
        The operator supplies security_id (example RELIANCE = 2885).
    """
    instrument = config["instrument"]
    if not instrument.get("security_id") or not (
        instrument.get("trading_symbol") or instrument.get("symbol")
    ):
        raise ConfigError("CONFIGURATION ERROR\ntrading_symbol and security_id are required")
    logging.info(
        "[SYSTEM] Instrument resolved: %s security_id=%s segment=%s product=%s qty=%s",
        instrument["symbol"],
...
def fetch_candles(dhan, config: dict) -> list[Candle]:
    """Fetch completed candles. Alias for get_market_data."""
    return get_market_data(dhan, config)


def fetch_market_data(dhan, config: dict) -> list[Candle]:
    """Fetch completed candles for indicator calculation."""
    return get_market_data(dhan, config)


def check_trading_session(config: dict, now: Optional[datetime] = None) -> tuple[bool, str]:
    """Whether new entries are allowed in the configured IST session."""
    return check_trading_window(config, now)


def check_duplicate_order(state: BotState, config: dict) -> bool:
    """
    True when a second entry must not be submitted.

    Duplicate-order protection is critical because the strategy is
    evaluated every few seconds. The same alignment may remain valid
...
def get_pending_orders(dhan, config: dict) -> list[dict]:
    """Working orders for the configured security."""
    return pending_orders_for_symbol(dhan, config)
...
def calculate_position_pnl(
    ...
    """Direction-aware P&L. Alias for calculate_pnl."""
    return calculate_pnl(side, quantity, entry_price, ltp)


def display_dashboard(config: dict, state: BotState) -> Panel:
    """Sci-fi CLI panel. Alias for render_dashboard."""
    return render_dashboard(config, state)


def check_stop_signal() -> bool:
    """True when stop.py created state/STOP or a process signal arrived."""
    return stop_requested()


def handle_shutdown(dhan, config: dict, state: BotState, reason: str) -> None:
    """Graceful shutdown. Alias for graceful_shutdown."""
    graceful_shutdown(dhan, config, state, reason)


def initialize_dhan():
    """Create the Dhan SDK client from environment credentials."""
    return create_dhan_client()


def get_historical_data(dhan, config: dict) -> list[Candle]:
    """Fetch completed candles. Alias for get_market_data."""
    return get_market_data(dhan, config)
...
def wait_for_order_fill(
    ...
    """Wait for a submitted order to reach a terminal broker state."""
...
def execute_signal(
    ...
    """
    Turn a LONG/SHORT strategy signal into one entry attempt.

    Why this function exists:
        Strategy math must stay separate from Dhan order submission.
    """
    if signal not in {"LONG", "SHORT"}:
        return False
    transaction = "BUY" if signal == "LONG" else "SELL"
    return place_entry_order(dhan, config, state, transaction, ltp)
...
def run_strategy_cycle(
    ...
    """One poll of data → indicators → signal → risk → orders → TP/SL."""
    return run_one_poll_cycle(dhan, config, state, cached_candles)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        logging.info("[SYSTEM] CONFIGURATION VALIDATED")
        ...
        state.trading_day = state.started_at.date().isoformat()
        load_persisted_state(config, state)
