#!/usr/bin/env python3
"""Dhan MACD Momentum Algo for NSE equity.
...
WHY_NO_TRADE_LABELS = {
    ...
    "NO_MACD_CROSSOVER": "No MACD crossover on this candle",
    "HISTOGRAM_FILTER": "Histogram confirmation failed",
    "TREND_FILTER": "Price is on the wrong side of the trend MA",
    "RSI_FILTER": "RSI confirmation failed",
    "VOLUME_FILTER": "Volume is below the configured threshold",
    "MACD_WARMUP": "MACD is still warming up",
    ...
    "SCANNING": "Flat — scanning for a MACD crossover",
...
@dataclass
class MacdPoint:
    """MACD values for one completed candle."""
...
class BotState:
    ...
    current_trend_ma: Optional[float] = None
    last_candle_time: Optional[datetime] = None
    last_candle_close: Optional[float] = None
    candle_bar_count: int = 0
    candle_bar_gap_minutes: Optional[float] = None
...
def parse_cli_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="MACD Momentum Algo for Dhan")
    parser.add_argument(
        "--config",
...
def validate_config(config: dict) -> None:
    ...
    bot["candle_lookback"] = int(
        _as_positive_number(bot.get("candle_lookback", 500), "bot.candle_lookback")
    )
    ...
    if not isinstance(macd, dict):
        raise ConfigError("ERROR: strategy.macd must be a mapping.\nThe bot did not start.")
    macd["fast_period"] = int(
        _as_positive_number(macd.get("fast_period"), "strategy.macd.fast_period")
    )
    macd["slow_period"] = int(
        _as_positive_number(macd.get("slow_period"), "strategy.macd.slow_period")
    )
    macd["signal_period"] = int(
        _as_positive_number(macd.get("signal_period"), "strategy.macd.signal_period")
    )
    if macd["fast_period"] >= macd["slow_period"]:
        raise ConfigError(
            "ERROR: strategy.macd.fast_period must be less than slow_period.\n"
            "The bot did not start."
        )
    strategy["macd"] = macd
    ...
    if not isinstance(filters, dict):
        raise ConfigError("ERROR: strategy.filters must be a mapping.\nThe bot did not start.")
    filters["histogram_confirmation"] = _as_bool(filters.get("histogram_confirmation"), False)
    filters["trend_filter_enabled"] = _as_bool(filters.get("trend_filter_enabled"), False)
    filters["trend_ma_period"] = int(
        _as_positive_number(filters.get("trend_ma_period", 50), "strategy.filters.trend_ma_period")
    )
    filters["rsi_filter_enabled"] = _as_bool(filters.get("rsi_filter_enabled"), False)
    filters["rsi_period"] = int(
        _as_positive_number(filters.get("rsi_period", 14), "strategy.filters.rsi_period")
    )
    filters["rsi_min"] = _as_non_negative_number(
        filters.get("rsi_min", 50), "strategy.filters.rsi_min"
    )
    filters["rsi_max"] = _as_non_negative_number(
        filters.get("rsi_max", 50), "strategy.filters.rsi_max"
    )
    if filters["rsi_min"] > 100 or filters["rsi_max"] > 100:
        raise ConfigError(
            "ERROR: RSI filter thresholds cannot exceed 100.\nThe bot did not start."
        )
    filters["volume_filter_enabled"] = _as_bool(filters.get("volume_filter_enabled"), False)
    filters["minimum_volume"] = _as_non_negative_number(
        filters.get("minimum_volume", 0), "strategy.filters.minimum_volume"
    )
    strategy["filters"] = filters

    needed = macd["slow_period"] + macd["signal_period"] + 2
    if filters["trend_filter_enabled"]:
        needed = max(needed, filters["trend_ma_period"])
    if filters["rsi_filter_enabled"]:
        needed = max(needed, filters["rsi_period"] + 1)
    ...
    sl_type = str(risk.get("stop_loss_type") or "PERCENT").strip().upper()
    tp_type = str(risk.get("take_profit_type") or "PERCENT").strip().upper()
    if sl_type not in VALID_TP_SL_TYPES or tp_type not in VALID_TP_SL_TYPES:
        raise ConfigError(
            "ERROR: risk.stop_loss_type and take_profit_type must be PERCENT or POINTS.\n"
            "The bot did not start."
        )
    risk["stop_loss_type"] = sl_type
    risk["take_profit_type"] = tp_type
    ...
    execution["allow_long"] = _as_bool(execution.get("allow_long"), True)
    execution["allow_short"] = _as_bool(execution.get("allow_short"), True)
    if not execution["allow_long"] and not execution["allow_short"]:
        raise ConfigError(
            "ERROR: at least one of execution.allow_long or allow_short must be true.\n"
            "The bot did not start."
        )
...
def validate_candles(candles: list[Candle]) -> list[Candle]:
    ...
    Why this function exists:
        A bad OHLC row can produce a fake MACD crossover. Unreliable data
...
def _bar_gap_minutes(first: Candle, second: Candle) -> float:
    return (second.timestamp - first.timestamp).total_seconds() / 60.0


def _median_bar_gap_minutes(candles: list[Candle]) -> Optional[float]:
    if len(candles) < 2:
        return None
    gaps = sorted(_bar_gap_minutes(candles[index - 1], candles[index]) for index in range(1, len(candles)))
    mid = len(gaps) // 2
    if len(gaps) % 2:
        return gaps[mid]
...
def apply_candle_diagnostics(state: BotState, candles: list[Candle]) -> None:
    """Store last close, bar count, and last-two-stamp gap for the dashboard."""
    if not candles:
        state.last_candle_close = None
        state.candle_bar_count = 0
        state.candle_bar_gap_minutes = None
        return
    latest = candles[-1]
    state.last_candle_time = latest.timestamp
    state.last_candle_close = latest.close
    state.candle_bar_count = len(candles)
    if len(candles) >= 2:
        state.candle_bar_gap_minutes = _bar_gap_minutes(candles[-2], latest)
    else:
        state.candle_bar_gap_minutes = None
...
def get_market_data(dhan, config: dict) -> list[Candle]:
    """
    Fetch required candle history for MACD.

    Why this function exists:
        MACD must be computed on closed candles. Fetching unbounded history
    ...
    last_stamp = completed[-1].timestamp.isoformat() if completed else "—"
    last_close = completed[-1].close if completed else None
    median_gap = _median_bar_gap_minutes(completed)
    logging.info(
        "Market data fetched: %s completed %s candles (lookback=%s) "
        "last=%s close=%s median_gap=%s min.",
        len(completed),
        timeframe,
        lookback,
        last_stamp,
        f"{last_close:.2f}" if last_close is not None else "—",
        f"{median_gap:.1f}" if median_gap is not None else "—",
    )
    return completed
...
def calculate_sma(closes: list[float], period: int) -> list[Optional[float]]:
    ...
    Why this function exists:
        The optional trend filter uses an SMA of price. Keeping it pure
        makes later backtests possible without Dhan.

    Returns:
        A list the same length as closes. Early values are None.
    """
...
def calculate_ema(values: list[float], period: int) -> list[Optional[float]]:
    """
    Exponential moving average using TradingView ``ta.ema`` recurrence.
    ...
    Returns:
        None until index ``period - 1`` so MACD warmup stays conservative.
        From that index the published value matches TradingView's running EMA.
...
def calculate_rsi(closes: list[float], period: int) -> list[Optional[float]]:
    """
    Wilder RSI used only as an optional confirmation filter.

    Why this function exists:
        The core signal is MACD. RSI here is a gate, not a standalone trigger.
    """
    size = len(closes)
...
def calculate_macd(
    ...
    first_valid = next((i for i, value in enumerate(macd_line) if value is not None), None)
    if first_valid is None:
        return points
    compact = [float(value) for value in macd_line[first_valid:] if value is not None]
    signal_compact = calculate_ema(compact, signal_period)
    for offset, signal_value in enumerate(signal_compact):
        index = first_valid + offset
        macd_value = macd_line[index]
        points[index].macd = macd_value
        points[index].signal = signal_value
        if macd_value is not None and signal_value is not None:
            points[index].histogram = macd_value - signal_value
    return points
...
def detect_crossover(
    ...
    current_signal: Optional[float],
) -> str:
    """
    Detect a MACD / Signal crossover as an event, not a standing level.
    ...
    Returns:
        LONG, SHORT, or NONE. Never places an order.
    """
    if (
...
def calculate_indicators(candles: list[Candle], config: dict) -> dict[str, Any]:
    """
    Compute every indicator the strategy and filters need.
    ...
    filters = config["strategy"]["filters"]
    points = calculate_macd(
        closes,
        int(macd_cfg["fast_period"]),
        int(macd_cfg["slow_period"]),
        int(macd_cfg["signal_period"]),
    )
    trend = (
        calculate_sma(closes, int(filters["trend_ma_period"]))
        if filters["trend_filter_enabled"]
        else [None] * len(closes)
    )
    rsi = (
        calculate_rsi(closes, int(filters["rsi_period"]))
        if filters["rsi_filter_enabled"]
        else [None] * len(closes)
    )
    return {"macd": points, "trend": trend, "rsi": rsi, "volumes": volumes, "closes": closes}
...
def generate_signal(
    ...
    """
    Generate a MACD momentum signal from completed candles.
    ...
    points: list[MacdPoint] = bundle["macd"]
    current = points[-1]
    previous = points[-2]
    last_candle = candles[-1]
    state.current_macd = current.macd
    state.current_signal = current.signal
    state.current_histogram = current.histogram
    state.previous_macd = previous.macd
    state.previous_signal = previous.signal
    state.current_rsi = bundle["rsi"][-1]
    state.current_trend_ma = bundle["trend"][-1]
    apply_candle_diagnostics(state, candles)

    if current.macd is None or current.signal is None or previous.macd is None or previous.signal is None:
        state.strategy_condition = "MACD_WARMUP"
        state.why_no_trade = "MACD_WARMUP"
        state.next_action = "WAITING FOR MACD WARMUP"
        return "NONE"

    side = detect_crossover(previous.macd, previous.signal, current.macd, current.signal)
    filters = config["strategy"]["filters"]

    if side == "NONE":
        state.strategy_condition = "NO_CROSSOVER"
        state.why_no_trade = "NO_MACD_CROSSOVER"
        state.next_action = "NO MACD CROSSOVER — HOLD / MONITOR"
        hist = current.histogram
        if hist is not None:
            state.strategy_condition = "BULLISH_HIST" if hist > 0 else "BEARISH_HIST" if hist < 0 else "FLAT_HIST"
        return "NONE"

    if filters["histogram_confirmation"]:
        hist = current.histogram
        if side == "LONG" and (hist is None or hist <= 0):
            state.strategy_condition = "LONG_CROSS_HIST_FAIL"
            state.why_no_trade = "HISTOGRAM_FILTER"
            state.confirmation_status = "HIST FAIL"
            return "NONE"
        if side == "SHORT" and (hist is None or hist >= 0):
            state.strategy_condition = "SHORT_CROSS_HIST_FAIL"
            state.why_no_trade = "HISTOGRAM_FILTER"
            state.confirmation_status = "HIST FAIL"
            return "NONE"

    if filters["trend_filter_enabled"]:
        trend_value = bundle["trend"][-1]
        price = last_candle.close
        if trend_value is None:
            state.strategy_condition = "TREND_WARMUP"
            state.why_no_trade = "TREND_FILTER"
            return "NONE"
        if side == "LONG" and price <= trend_value:
            state.strategy_condition = "LONG_AGAINST_TREND"
            state.why_no_trade = "TREND_FILTER"
            state.confirmation_status = "TREND FAIL"
            return "NONE"
        if side == "SHORT" and price >= trend_value:
            state.strategy_condition = "SHORT_AGAINST_TREND"
            state.why_no_trade = "TREND_FILTER"
            state.confirmation_status = "TREND FAIL"
            return "NONE"

    if filters["rsi_filter_enabled"]:
        rsi_value = bundle["rsi"][-1]
        if rsi_value is None:
            state.strategy_condition = "RSI_WARMUP"
            state.why_no_trade = "RSI_FILTER"
            return "NONE"
        if side == "LONG" and rsi_value < float(filters["rsi_min"]):
            state.strategy_condition = "LONG_RSI_FAIL"
            state.why_no_trade = "RSI_FILTER"
            state.confirmation_status = "RSI FAIL"
            return "NONE"
        if side == "SHORT" and rsi_value > float(filters["rsi_max"]):
            state.strategy_condition = "SHORT_RSI_FAIL"
            state.why_no_trade = "RSI_FILTER"
            state.confirmation_status = "RSI FAIL"
            return "NONE"

    if filters["volume_filter_enabled"]:
        if last_candle.volume < float(filters["minimum_volume"]):
            state.strategy_condition = "VOLUME_FAIL"
            state.why_no_trade = "VOLUME_FILTER"
            state.confirmation_status = "VOLUME FAIL"
            return "NONE"

    state.last_signal = side
    state.confirmation_status = "CONFIRMED"
    state.strategy_condition = f"{side}_SIGNAL"
    return side
...
def validate_trade(config: dict, state: BotState, signal: str) -> tuple[bool, str]:
    """
    Verify that a MACD signal is allowed to become an order.
    ...
    if config["execution"].get("prevent_duplicate_orders", True):
        ...
        if state.last_processed_candle is not None and state.last_signal in {"LONG", "SHORT"}:
    ...
    if signal == "LONG" and not config["execution"].get("allow_long"):
        return False, "LONG_DISABLED"
    if signal == "SHORT" and not config["execution"].get("allow_short"):
...
def render_startup_banner(config: dict) -> None:
    ...
    grid.add_row("BOT", str(config["bot"].get("name") or "MACD Momentum Algo"))
    ...
    grid.add_row("MACD", f"{macd['fast_period']}/{macd['slow_period']}/{macd['signal_period']}")
    grid.add_row("SL", f"{risk['stop_loss']} {risk['stop_loss_type']}")
    grid.add_row("TP", f"{risk['take_profit']} {risk['take_profit_type']}")
...
def render_dashboard(config: dict, state: BotState) -> Panel:
    ...
    macd_text = f"{state.current_macd:.4f}" if state.current_macd is not None else "—"
    signal_text = f"{state.current_signal:.4f}" if state.current_signal is not None else "—"
    hist = state.current_histogram
    hist_text = f"{hist:+.4f}" if hist is not None else "—"
    trend = "BULLISH" if hist is not None and hist > 0 else "BEARISH" if hist is not None and hist < 0 else "—"
    rsi_text = f"{state.current_rsi:.2f}" if state.current_rsi is not None else "—"
    trend_ma = f"{state.current_trend_ma:.2f}" if state.current_trend_ma is not None else "—"
    candle_text = (
        state.last_candle_time.strftime("%H:%M:%S") if state.last_candle_time else "—"
    )
    bars_text = str(state.candle_bar_count) if state.candle_bar_count else "—"
    if state.candle_bar_gap_minutes is None:
        gap_text = "—"
    elif abs(state.candle_bar_gap_minutes - round(state.candle_bar_gap_minutes)) < 1e-6:
        gap_text = f"{int(round(state.candle_bar_gap_minutes))}m"
    else:
        gap_text = f"{state.candle_bar_gap_minutes:.1f}m"
    ...
    market_tbl.add_row("CLOSE", format_inr(state.last_candle_close), "BARS", bars_text)
    market_tbl.add_row("CANDLE", candle_text, "BAR GAP", gap_text)
    if cli.get("show_indicators", True):
        market_tbl.add_row("MACD", macd_text, "SIGNAL", signal_text)
        market_tbl.add_row("HIST", hist_text, "TREND", trend)
        if strategy["filters"]["rsi_filter_enabled"]:
            market_tbl.add_row("RSI", rsi_text, "TREND MA", trend_ma)
        elif strategy["filters"]["trend_filter_enabled"]:
            market_tbl.add_row("TREND MA", trend_ma, "", "")
    data_at = (
        state.last_market_data_at.strftime("%H:%M:%S") if state.last_market_data_at else "—"
    )
    market_tbl.add_row("DATA AT", data_at, "", "")
    ...
    return Panel(
        Group(*sections),
        title="[bold cyan]◈ MACD MOMENTUM // DHAN CORE ◈[/bold cyan]",
...
def process_new_candle(
    ...
    if state.last_processed_candle is not None and latest.timestamp <= state.last_processed_candle:
        ...
        bundle = calculate_indicators(candles, config)
        point = bundle["macd"][-1]
        state.current_macd = point.macd
        state.current_signal = point.signal
        state.current_histogram = point.histogram
        state.current_rsi = bundle["rsi"][-1]
        state.current_trend_ma = bundle["trend"][-1]
    ...
    apply_candle_diagnostics(state, candles)
    if state.current_macd is not None:
        logging.info("MACD CALCULATED")
        push_event(
            "MACD",
            f"{state.current_macd:.4f} / {state.current_signal:.4f} / {state.current_histogram}",
        )
    ...
    if signal in {"LONG", "SHORT"}:
        ...
        logging.info("SIGNAL DETECTED: MACD %s CROSSOVER", signal)
        push_event("SIGNAL", f"MACD {signal} CROSSOVER")
...
def run_one_poll_cycle(
    ...
    # Fetch candles even when entries are blocked (weekend / session / risk)
    # so the dashboard can show MACD. process_new_candle will not order.
